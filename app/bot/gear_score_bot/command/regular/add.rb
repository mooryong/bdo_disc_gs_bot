class GearScoreBot::Command::Regular::Add < GearScoreBot::Command::Base
  ARG_INDEX_MAP = {
    family_name: 0,
    character_name: 1,
    preawakening_ap: 2,
  ...
  def find_or_create_user!
    @_user ||= User.where(discord_id: @_user_id).first_or_create!
  end

  def find_or_create_character!
    @_character ||= Character.where(
      family_name: @_formatted_data[:family_name], 
      character_name: @_formatted_data[:character_name], 
      discord_id: @_user_id
    ).first_or_create! do |character|
      character.update_attributes!(
        @_formatted_data.slice(
          :preawakening_attack_power,
          :awakening_attack_power,
          :defense_power,
          :level,
          :class_name,
          :image_url
